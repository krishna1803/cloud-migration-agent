...
class OCIConfig(BaseSettings):
    ...
    tenancy_id: str = Field(alias="OCI_TENANCY_ID")
    user_id: str = Field(alias="OCI_USER_ID")
    fingerprint: str = Field(alias="OCI_FINGERPRINT")
    private_key_path: str = Field(alias="OCI_PRIVATE_KEY_PATH")
    compartment_id: str = Field(alias="OCI_COMPARTMENT_ID")
...
class GenAIConfig(BaseSettings):
    ...
    endpoint: str = Field(alias="OCI_GENAI_ENDPOINT")
    ...
    temperature: float = Field(default=0.7, alias="OCI_GENAI_TEMPERATURE")
...
class DatabaseConfig(BaseSettings):
    ...
    user: str = Field(alias="ORACLE_DB_USER")
    password: str = Field(alias="ORACLE_DB_PASSWORD")
...
class AppConfig(BaseSettings):
    ...
    checkpoint_enabled: bool = Field(default=True, alias="CHECKPOINT_ENABLED")
    ...
    feature_cost_optimization: bool = Field(
        default=True,
        alias="FEATURE_COST_OPTIMIZATION"
    )
    feature_kb_rag: bool = Field(default=True, alias="FEATURE_KB_RAG")
...
class MCPConfig(BaseSettings):
    ...
    docs_server_url: str = Field(
        default="http://localhost:8001",
        alias="MCP_DOCS_SERVER_URL"
    )
    kb_server_url: str = Field(
        default="http://localhost:8002",
        alias="MCP_KB_SERVER_URL"
    )
    mapping_server_url: str = Field(
        default="http://localhost:8003",
        alias="MCP_MAPPING_SERVER_URL"
    )
    sizing_server_url: str = Field(
        default="http://localhost:8004",
        alias="MCP_SIZING_SERVER_URL"
    )
    pricing_server_url: str = Field(
        default="http://localhost:8005",
        alias="MCP_PRICING_SERVER_URL"
    )
    terraform_server_url: str = Field(
        default="http://localhost:8006",
        alias="MCP_TERRAFORM_SERVER_URL"
    )
    deployment_server_url: str = Field(
        default="http://localhost:8007",
        alias="MCP_DEPLOYMENT_SERVER_URL"
    )
    validation_server_url: str = Field(
        default="http://localhost:8008",
        alias="MCP_VALIDATION_SERVER_URL"
    )
    monitoring_server_url: str = Field(
        default="http://localhost:8009",
        alias="MCP_MONITORING_SERVER_URL"
    )
    reporting_server_url: str = Field(
        default="http://localhost:8010",
        alias="MCP_REPORTING_SERVER_URL"
    )
